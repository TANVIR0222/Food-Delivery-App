...
export default function OrdersScreen() {
  ...
  const renderOrderItem = ({ item }: any) => (
    <View
      style={tw`flex-row bg-description2/10 rounded-2xl p-4 mb-4 items-center gap-3`}
    >
      <Image
        source={item.image}
      ...
      <View style={tw`flex-1`}>
        <Text style={tw`text-black font-inter-bold text-base`}>
          {item.name}
        </Text>
      ...
      <View style={tw`flex-col items-center gap-2`}>
        <View
          style={tw`flex-row items-center gap-2 bg-stroke rounded-lg px-2 py-1`}
        >
          <TouchableOpacity
        ...
        <TouchableOpacity
          ...
          style={tw`bg-white rounded-lg p-2`}
        >
    ...
    <View style={tw`flex-1 pt-[${top}px] bg-white`}>
      <PageWrapper>
        <FlatList
          ...
          ListHeaderComponent={
            <Text style={tw`text-black text-xl py-3 font-inter-bold mb-4`}>
